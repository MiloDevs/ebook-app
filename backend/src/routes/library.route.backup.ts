...
const validateCreateLibrary = (body: any) => {
...
const validateAddBookToLibrary = (body: any) => {
...
const app = new Hono<AppEnv>()
  ...
  // Create new library
  .post("/:userId", withPrisma, async (c) => {
    try {
      const prisma = c.get("prisma");
      const userId = c.req.param("userId");
      ...
      try {
        ({ name, description } = validateCreateLibrary(body));
      } catch (validationError) {
        return c.json({ error: (validationError as Error).message }, 400);
      }
  ...
  // Update library
  .put("/:userId/:libraryId", withPrisma, async (c) => {
    try {
      const prisma = c.get("prisma");
      const userId = c.req.param("userId");
      const libraryId = c.req.param("libraryId");
      const body = await c.req.json();
      const { name, description } = validateCreateLibrary(body);
  ...
  // Add book to library
  .post("/:userId/:libraryId/books", withPrisma, async (c) => {
    try {
      const prisma = c.get("prisma");
      const userId = c.req.param("userId");
      const libraryId = c.req.param("libraryId");
      const body = await c.req.json();
      const { book_id } = validateAddBookToLibrary(body);
